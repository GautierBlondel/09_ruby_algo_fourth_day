...
datas.length
  # puts "datas.size #{datas.size}"

# Quel est le handle le plus court de cette liste ?

datas.each do |key|
  # puts "data #{key}"
  #  trouver la plus petite chaine de caractère / chercher méthode
end
...
# Combien commencent par une majuscule (première lettre juste après le @) ?
  ...
  count = 0
  datas.each do |data|
    letter_copy = data.split("")[1]
    if data.split("")[1] == letter_copy.upcase
      # puts "letter_copy #{letter_copy}"
      count = count + 1
      # puts "count #{count}"
    end
  end
...
sort_arr = datas.sort { |w1, w2| w1.casecmp(w2) }
# puts "sort_arr #{sort_arr}"

# Trie la liste de handle par taille des handle (les plus petits en premiers, les plus grands après)
length_arr = datas.sort_by {|x| x.length}

# puts "length_arr #{length_arr}"

# Quelle est la position dans l'array de la personne @epenser ?
think_index = datas.index("@epenser")
# puts "think_index #{think_index}"
...
def length_list(datas)
  length_arr = datas.sort_by {|x| x.length}
  table_obj = Hash.new
  
  length_arr.each_with_index do |data, i|
      # puts "i #{i}"
    # i = i + 1 
    case table_obj.has_key?("length: #{data.length}")
    when false
      table_obj["length: #{data.length}"] = 1
    when true
      table_obj["length: #{data.length}"] = table_obj["length: #{data.length}"] + 1 
    end
  end
  return table_obj
end
